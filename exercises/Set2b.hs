...
binomial 0 k
    | k > 0 = 0
    | otherwise = binomial (-1) k + binomial (-1) (k-1)
...
countdown n = "Ready! " ++ numberCountdown n ++ "Liftoff!"

numberCountdown :: Integer -> String
numberCountdown 0 = ""
numberCountdown n = show n ++ "... " ++ numberCountdown (n-1)
...
smallestDivisor n = divide n 2

divide :: Integer -> Integer -> Integer
divide n y
    | mod n y == 0 = y
    | otherwise = divide n (y+1)
...
biggestPrimeAtMost n
    | checkPrime = n
    | not checkPrime = biggestPrimeAtMost (n-1)
    where checkPrime = isPrime n
