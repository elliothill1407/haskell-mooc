...
whileRight' :: (t -> Either a t) -> Either a t -> a
...
joinToLength n lst = [str | a <- lst, b <- lst, let str = a ++ b, length str == n]
...
interpreter' [] _ _ = []
interpreter' (x:xs) xC yC
    | x == "up"     = interpreter' xs xC (yC+1)
    | x == "down"   = interpreter' xs xC (yC-1)
    | x == "left"   = interpreter' xs (xC-1) yC
    | x == "right"  = interpreter' xs (xC+1) yC
    | x == "printX" = show xC : interpreter' xs xC yC
    | x == "printY" = show yC : interpreter' xs xC yC