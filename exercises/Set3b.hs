...
sums' i c cSum
  | c > i     = []
  | otherwise = cSum : sums' i (c + 1) (cSum + c + 1)
...
sumsOf' [] _ = []
sumsOf' [x] sum = [sum + x]
sumsOf' (y:x:xs) sum = sum + y : sumsOf' (x:xs) (sum + y)
...
mymaximum bigger initial [x]
    | bigger initial x = mymaximum bigger initial []
    | otherwise = x
mymaximum bigger initial (y:x:xs)
    | bigger y x = mymaximum bigger initial (y:xs)
    | bigger x y = mymaximum bigger initial (x:xs)
    | otherwise  = mymaximum bigger initial (x:xs)
...
maybeMap = maybeFilter

maybeFilter f [] = []
maybeFilter f (x:xs)
    | isJust (f x) = fromJust (f x) : maybeFilter f xs
    | otherwise    = maybeFilter f xs
